...
templater.ensureMustacheVariables = function (item, caseMethod) {
  // Define strings in the appropriate case
  // TODO: Accept `options` for `case` via second param
  item.strings = {
    // icon-home
    name: _s[caseMethod](item.name),
    // icon-home-name
    name_name: _s[caseMethod](item.name + '-name'),
    // icon-home-x
    name_x: _s[caseMethod](item.name + '-x'),
    // icon-home-y
    name_y: _s[caseMethod](item.name + '-y'),
    // icon-home-offset-x
    name_offset_x: _s[caseMethod](item.name + '-offset-x'),
    // icon-home-offset-y
    name_offset_y: _s[caseMethod](item.name + '-offset-y'),
    // icon-home-width
    name_width: _s[caseMethod](item.name + '-width'),
    // icon-home-height
    name_height: _s[caseMethod](item.name + '-height'),
    // icon-home-total-width
    name_total_width: _s[caseMethod](item.name + '-total-width'),
    // icon-home-total-height
    name_total_height: _s[caseMethod](item.name + '-total-height'),
    // icon-home-image
    name_image: _s[caseMethod](item.name + '-image'),
    // icon-home-sprites
    name_sprites: _s[caseMethod](item.name + '-sprites'),

    // Bare strings for maps
    bare_name: _s[caseMethod]('name'),
    bare_x: _s[caseMethod]('x'),
    bare_y: _s[caseMethod]('y'),
    // TODO: _s[caseMethod](Move back to dash-case as defaul)t
    bare_offset_x: _s[caseMethod]('offset_x'),
    bare_offset_y: _s[caseMethod]('offset_y'),
    bare_width: _s[caseMethod]('width'),
    bare_height: _s[caseMethod]('height'),
    bare_total_width: _s[caseMethod]('total_width'),
    bare_total_height: _s[caseMethod]('total_height'),
    bare_image: _s[caseMethod]('image'),
    bare_sprites: _s[caseMethod]('sprites')
  };
};
...
templater.addMustacheTemplate = function (name, tmplStr) {
  ...
  function templateFn(itemObj) {
    ...
    var caseMethod = itemObj.options.caseMethod || 'dasherize';
    templater.ensureMustacheVariables(itemObj.spritesheet, caseMethod);
    itemObj.items.forEach(function addMustacheVariables (item) {
      templater.ensureMustacheVariables(item, caseMethod);
    });
