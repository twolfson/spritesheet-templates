...
function templater(params, options) {
  ...
  // TODO: Generate `strings` for template itself (e.g. for mixins in scss maps)
...
templater.ensureTemplateVariables = function (item) {
  ...
  item.strings = {
    ...
    name: item.name,
    // icon-home-name
    name_name: item.name + '-name',
    // icon-home-x
    name_x: item.name + '-x',
    // icon-home-y
    name_y: item.name + '-y',
    // icon-home-offset-x
    name_offset_x: item.name + '-offset-x',
    // icon-home-offset-y
    name_offset_y: item.name + '-offset-y',
    // icon-home-width
    name_width: item.name + '-width',
    // icon-home-height
    name_height: item.name + '-height',
    // icon-home-total-width
    name_total_width: item.name + '-total-width',
    // icon-home-total-height
    name_total_height: item.name + '-total-height',
    // icon-home-image
    name_image: item.name + '-image',
    // icon-home-sprites
    name_sprites: item.name + '-sprites',

    // Bare strings for maps
    bare_name: 'name',
    bare_x: 'x',
    bare_y: 'y',
    // TODO: Move back to dash-case as default
    bare_offset_x: 'offset_x',
    bare_offset_y: 'offset_y',
    bare_width: 'width',
    bare_height: 'height',
    bare_total_width: 'total_width',
    bare_total_height: 'total_height',
    bare_image: 'image',
    bare_sprites: 'sprites'
  };
};
