...
function templater(data, options) {
  ...
  if (data.retina_sprites || data.retina_spritesheet || data.retina_groups) {
    ...
    retinaSpritesheetInfo.name = retinaSpritesheetInfo.name || 'spritesheet-retina';
    ...
    // Add on normal/retina sprites
    // DEV: If we ever need to add double names for groups (e.g. `name1_group: name2`)
    //   then add `var_name` or similar as a breaking change in `spritesmith` for renaming
    retinaGroups.forEach(function getGroupSprites (group) {
      // Access the normal/retina sprites by index to make all new properties transfer by reference
      // DEV: If `group.index` ever needs to split on a per-normal/retina basis, add `group.normal_index/retina_index`
      group.normal = sprites[group.index];
      group.retina = retinaSprites[group.index];
    });
...
templater.ensureHandlebarsVariables = function (item, transformFn) {
  ...
  if (item.name) {
    _.extend(item.strings, {
      ...
      // icon-home-group
      name_sprites_group: transformFn(item.name + '-group'),
      // icon-home-group-name
      name_sprites_group_name: transformFn(item.name + '-group-name')
    });
  }
