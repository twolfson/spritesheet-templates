...
templater.addHandlebarsTemplate = function (name, tmplStr) {
  ...
  function templateFn(data) {
    ...
    if (data.retina_pairs) {
      data.retina_pairs.forEach(function addHandlebarsVariables (retinaPair) {
        templater.ensureHandlebarsVariables(retinaPair, transformFn);
        templater.ensureHandlebarsVariables(retinaPair.normal, transformFn);
        templater.ensureHandlebarsVariables(retinaPair.retina, transformFn);
      });
    }
...
// TODO: We need to use `snake_case` for filenames
var scssRetinaHandlebars = fs.readFileSync(templatesDir + '/scss-retina.template.handlebars', 'utf8');
templater.addHandlebarsTemplate('scss-retina', scssRetinaHandlebars);
