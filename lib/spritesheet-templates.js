...
function templater(data, options) {
  ...
  if (data.retinaSprites || data.retinaSpritesheet || data.retinaGroups) {
    // Verify we have all our parameters and deep clone them
    assert(data.retinaSprites && data.retinaSpritesheet && data.retinaGroups,
      'Expected "data.retinaSprites", "data.retinaSpritesheet", and "data.retinaGroups" to be provided ' +
      'but at least one of them was missing');
    retinaSprites = JSON.parse(JSON.stringify(data.retinaSprites));
    retinaSpritesheet = JSON.parse(JSON.stringify(data.retinaSpritesheet));
    // TODO: Use fix for `_.extend`
    // TODO: Take in `retina_spritesheet_info/retina_groups_info` via `data`, not `options`
    retinaSpritesheetInfo = _.extend({
      name: 'spritesheet-retina'
    }, options.retinaSpritesheetInfo);
    retinaSpritesheetInfo = JSON.parse(JSON.stringify(retinaSpritesheetInfo));
    retinaGroups = JSON.parse(JSON.stringify(data.retinaGroups));
    retinaGroupsInfo = _.extend({
      name: 'retina-sprites'
    }, options.retinaGroupsInfo);
    retinaGroupsInfo = JSON.parse(JSON.stringify(retinaGroupsInfo));

    // Map retina groups to their counterparts
    retinaGroups = retinaGroups.map(function getGroupSprites (group) {
      // Access the normal/retina sprites by index to make all new properties transfer easily
      return _.defaults({
        normal: sprites[group.normal],
        retina: retinaSprites[group.retina]
      }, group);
    });

    // Enhance the properties of our sprites
  ...
  function templateFn(data) {
    ...
    if (data.retina_groups) {
      data.retina_groups.forEach(function addHandlebarsVariables (retinaGroup) {
        ...
        templater.ensureHandlebarsVariables(retinaGroup.normal, transformFn);
        templater.ensureHandlebarsVariables(retinaGroup.retina, transformFn);
      });
    }
