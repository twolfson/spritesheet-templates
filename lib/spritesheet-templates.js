...
function templater(params, options) {
  ...
  // If we have retina items, enhance them as well well
  var retinaItems;
  var retinaSpritesheet;
  if (params.retinaItems || params.retinaSpritesheet) {
    assert(params.retinaItems && params.retinaSpritesheet, 'Expected both "params.retinaItems" and ' +
      '"params.retinaSpritesheet" to be provided but one of them was missing');
    retinaItems = JSON.parse(JSON.stringify(params.retinaItems));
    ...
    retinaItems.forEach(function addExtraData (retinaItem) {
      templater.addSpritesheetProperties(retinaItem, retinaSpritesheet);
      templater.ensureTemplateVariables(retinaItem);
    });
  }
...
templater.addHandlebarsTemplate = function (name, tmplStr) {
  ...
  function templateFn(itemObj) {
    ...
    if (itemObj.retina_spritesheet) {
      templater.ensureMustacheVariables(itemObj.retina_spritesheet, transformFn);
    }
    if (itemObj.retina_items) {
      itemObj.retina_items.forEach(function addMustacheVariables (retinaItem) {
        templater.ensureMustacheVariables(retinaItem, transformFn);
      });
    }

    // Render the items via the template
    var retStr = handlebars.compile(tmpl)(itemObj);
    return retStr;
  }
