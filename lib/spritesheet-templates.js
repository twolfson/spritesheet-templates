...
templater.ensureHandlebarsVariables = function (item, transformFn) {
  ...
  if (item.name) {
    // icon-home@2x -> icon-home-2x
    var slugName = slug(item.name);
    _.extend(item.strings, {
      // icon-home
      name: transformFn(slugName),
      // icon-home-name
      name_name: transformFn(slugName + '-name'),
      // icon-home-x
      name_x: transformFn(slugName + '-x'),
      // icon-home-y
      name_y: transformFn(slugName + '-y'),
      // icon-home-offset-x
      name_offset_x: transformFn(slugName + '-offset-x'),
      // icon-home-offset-y
      name_offset_y: transformFn(slugName + '-offset-y'),
      // icon-home-width
      name_width: transformFn(slugName + '-width'),
      // icon-home-height
      name_height: transformFn(slugName + '-height'),
      // icon-home-total-width
      name_total_width: transformFn(slugName + '-total-width'),
      // icon-home-total-height
      name_total_height: transformFn(slugName + '-total-height'),
      // icon-home-image
      name_image: transformFn(slugName + '-image'),
      // icon-home-sprites
      name_sprites: transformFn(slugName + '-sprites'),
      // icon-home-group
      name_group: transformFn(slugName + '-group'),
      // icon-home-group-name
      name_group_name: transformFn(slugName + '-group-name'),
      // icon-home-normal
      name_normal: transformFn(slugName + '-normal'),
      // icon-home-retina
      name_retina: transformFn(slugName + '-retina')
    });
  }
