...
function templater(params, options) {
  ...
  // DEV: We have `spritesheetInfo` which is the object replacement option for `spritesheetName`
  var spritesheetInfo = _.extend({
    name: options.spritesheetName || 'spritesheet'
  }, options.spritesheetInfo);
  ...
  if (params.retinaSprites || params.retinaSpritesheet || params.retinaGroups) {
    // Verify we have all our parameters and deep clone them
    assert(params.retinaSprites && params.retinaSpritesheet && params.retinaGroups,
      'Expected "params.retinaSprites", "params.retinaSpritesheet", and "params.retinaGroups" to be provided ' +
    ...
    retinaGroups = JSON.parse(JSON.stringify(params.retinaGroups));
  ...
  var retVal = template({
    ...
    // TODO: Make `spritesheet_name` deprecation a separate release
